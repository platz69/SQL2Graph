...
CREATE TABLE T_REGION (
    ...
    id_pays    INT          NOT NULL REFERENCES T_PAYS(id_pays),  -- cardinalité 1..N (PAYS -> REGION) -- FK simple	NOT NULL déclarée au niveau colonne
...
CREATE TABLE T_VILLE (
    ...
    CONSTRAINT FK_VILLE_REGION FOREIGN KEY (id_region) REFERENCES T_REGION(id_region)     -- FK simple	NOT NULL déclarée au niveau table
);
...
CREATE TABLE T_CLIENT (
    ...
    id_produit_favori INT        NULL REFERENCES T_PRODUIT(id_produit),           -- FK simple	NULL     déclarée au niveau colonne
    ...
    CONSTRAINT FK_CLIENT_VILLE_FACT FOREIGN KEY (id_region_fact, code_ville_fact) -- FK composite NULL	 déclarée au niveau table
        REFERENCES T_VILLE(id_region, code_ville)
);

CREATE TABLE T_COMPTE_CLIENT (
    id_client      INT         NOT NULL PRIMARY KEY              -- PK simple	 déclarée au niveau colonne
                                REFERENCES T_CLIENT(id_client),  -- cardinalité 1..1 : PK = FK (clé partagée) -- FK simple	NOT NULL déclarée au niveau colonne
...
CREATE TABLE T_COMMANDE (
    ...
    CONSTRAINT FK_COMMANDE_PRODUIT_PROMO FOREIGN KEY (id_produit_promo)
        REFERENCES T_PRODUIT(id_produit)          -- FK simple	NULL     déclarée au niveau table
);

CREATE TABLE T_LIGNE_COMMANDE (
    id_commande   INT         NOT NULL,
    numero_ligne  INT         NOT NULL,
    id_produit    INT         NOT NULL REFERENCES T_PRODUIT(id_produit),  -- FK simple	NOT NULL déclarée au niveau colonne
    id_region     INT         NOT NULL,
    code_ville    VARCHAR(10) NOT NULL,
...
ALTER TABLE T_CLIENT
    ADD CONSTRAINT FK_CLIENT_PAYS FOREIGN KEY (id_pays) -- FK simple	NULL     déclarée au niveau ALTER
        REFERENCES T_PAYS(id_pays);                     -- cardinalité 0..N
...
ALTER TABLE T_COMMANDE
    ADD CONSTRAINT FK_COMMANDE_CLIENT FOREIGN KEY (id_client) -- FK simple	NOT NULL déclarée au niveau ALTER
        REFERENCES T_CLIENT(id_client);                       -- cardinalité 1..N
...
ALTER TABLE T_LIGNE_COMMANDE
    ADD CONSTRAINT FK_LIGNE_COMMANDE FOREIGN KEY (id_commande) -- FK simple	NOT NULL déclarée au niveau ALTER
        REFERENCES T_COMMANDE(id_commande);                    -- complète la table d'association N..N

ALTER TABLE T_COMMANDE
    ADD CONSTRAINT FK_COMMANDE_VILLE_LIVRAISON FOREIGN KEY (id_region_livraison, code_ville_livraison)
        REFERENCES T_VILLE(id_region, code_ville);   -- FK composite	NULL	 déclarée au niveau ALTER
